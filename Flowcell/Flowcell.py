...
class Camera_flowcell():
    ...
    def camera_connect(self):
        ...
        self.cap.set(3, self.pic_lenght)
    ...
    def take_video(self, frame_number=20):
        """
        ...
        # defines the number of frames to collect
        currentframe = 0
    ...
    def take_calibration_photo(self):
        """
        Connects to camera and takes a single picture,
    ...
    def thresholding(self, gray_scale, window_size, additional_thresh, i):
        """
        Applies the sauvola thresholding to the image, then converts it to a binary image,
    ...
    def particle_analysis(self):
        ...
        # find the two rectangles used for calibration
        ...
        # which have been measured manually with a microscope
        ...
        for file in glob.glob(path):
            ...
            # subtracting the two pictures to exclude the calibration picture
            ...
            # most of the particles can be detected and no areas are falsely detected
        ...
        # ___Analysis with area equivalent diameter___________________________
        Stationary_Data = dataframe.sort_values(
    ...
    def xslx_save(self, df):
        ...
        """
        # check to see of the xlsx file exists
        if not os.path.exists('PSD.xlsx'):
            with pd.ExcelWriter('PSD.xlsx', mode='w', if_sheet_exists='new') as writer:
        ...
        while index == False:
            ...
            while True:
                try:
                    ...
                    self.Logger.log(
                        "Camera Error, please check camera connection")
                    time.sleep(1)
                    continue
            ...
            # makes sure that the analysis loop takes at least 5 minutes
            if stat_t_diff < 300:
                time.sleep(300-stat_t_diff)
            i += 1
...
if __name__ == "__main__":
    ...
    camera.take_calibration_photo()
    camera.take_video(10)
    PSD_Data, Stationary_Data, n_total = camera.particle_analysis()
    camera.Logger.log(PSD_Data.head())
